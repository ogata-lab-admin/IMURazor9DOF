...
#define PI 3.1415926
	...
	// 2G mode
	m_accel.data.ax = rax / 255.0 * G; // m/sec^2
	m_accel.data.ay = ray / 255.0 * G;
	m_accel.data.az = raz / 255.0 * G;
	::setTimestamp<RTC::TimedAcceleration3D>(m_accel);
	m_accelOut.write();

	m_angularVel.data.avx = rwx / ((double)(0x7FFF)) * 2000.0 / 180 * PI; // rad/sec
	m_angularVel.data.avy = rwy / ((double)(0x7FFF)) * 2000.0 / 180 * PI;
	m_angularVel.data.avz = rwz / ((double)(0x7FFF)) * 2000.0 / 180 * PI;
